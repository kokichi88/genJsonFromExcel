...
namespace Core.Skills.Modifiers.Info {
	public class LauncherInfo : KnockdownInfo {
		private float speedForUpperAnimation;

		public LauncherInfo(Target target, float successRate, float delayToApply, WeightLevel level, float height,
		                    float timeToPeak, float timeToGround, float lieToIdleDuration, float distance, float lieDuration,
		                    float speedForUpperAnimation, float floatingDur, bool enableWallHit,
			...
			this.speedForUpperAnimation = speedForUpperAnimation;
		}
		...
		public float SpeedForUpperAnimation {
			get { return speedForUpperAnimation; }
		}
	}
}